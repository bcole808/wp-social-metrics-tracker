...
* @group selenium
*/
class TestSettings extends PHPUnit_Extensions_Selenium2TestCase {
	...
	/***************************************************
	* Make sure that each settings page can be saved and it does not overwrite other pages
	***************************************************/
	function test_settings_are_saved() {

		// Install DB
		shell_exec('rake test:install_single');
		...
		$this->go_to_settings_page('API Connection Settings');
		$this->change_settings_api();
		
		$this->go_to_settings_page('Advanced Domain / URL Setup');
		$this->change_settings_domain();

		// ASSERTIONS
		$this->go_to_settings_page('General Settings');
	...
	***************************************************/
	...
	function go_to_settings_page($section=false) {
		$menuitem = $this->byCssSelector('#toplevel_page_social-metrics-tracker');
		$menuitem->click();

		$thing = $this->byLinkText("Configuration");
		$thing->click();
