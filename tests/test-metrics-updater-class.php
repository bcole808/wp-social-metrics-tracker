...
class MetricUpdaterTests extends WP_UnitTestCase {
	...
	function assert_correct_data($post_id, $skip_facebook = false) {
		// Facebook
		if (!$skip_facebook) {
			$this->assertEquals(71753, get_post_meta($post_id, 'socialcount_facebook', true));
		...
		if ($skip_facebook) {
			...
			$this->assertEquals(221830, get_post_meta($post_id, 'socialcount_TOTAL', true));
		}
		...
		if ($skip_facebook) {
			...
			$this->assertEquals(221830, get_post_meta($post_id, 'social_aggregate_score', true));
